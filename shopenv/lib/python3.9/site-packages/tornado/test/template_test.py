...
three{%end%}
    ...
    def test_error_line_number_module(self):
        ...
        loader = DictLoader(
            {"profile.html": "{% module Template('sub.html') %}", "sub.html": "{{1/0}}"},
            namespace={"_tt_modules": ObjectDict(Template=load_generate)},
        )
        try:
            loader.load("profile.html").generate()
            self.fail("did not get expected exception")
        except ZeroDivisionError:
            exc_stack = traceback.format_exc()
            self.assertTrue("# profile.html:1" in exc_stack)
            self.assertTrue("# sub.html:1" in exc_stack)

    def test_error_line_number_include(self):
        loader = DictLoader(
            {"profile.html": "{% include 'sub.html' %}", "sub.html": "{{1/0}}"}
        )
        try:
            loader.load("profile.html").generate()
            self.fail("did not get expected exception")
        except ZeroDivisionError:
            self.assertTrue("# sub.html:1 (via profile.html:1)" in traceback.format_exc())

    def test_error_line_number_extends_base_error(self):
        loader = DictLoader(
            {"profile.html": "{{1/0}}", "sub.html": "{% extends 'profile.html' %}"}
        )
        try:
...
{% end %}
        ...
        try:
            ...
            self.assertTrue("# sub.html:4 (via profile.html:1)" in traceback.format_exc())
